package it.polito.security.esp.optimizationAPI;
...
public class CPlexOptimizer implements Optimizer {
	...
	@Override
	public int getNumberOfSolutions() throws OptimizerException {
		try {
	...
	@Override
	public Double evaluateExpressionWithBestSolution(Object expr) throws OptimizerException
	{
		try	{
			if(expr instanceof IloNumExpr)
				return cplex.getValue((IloNumExpr) expr);
			else if(expr instanceof IloNumVar)
				return cplex.getValue((IloNumVar) expr);
			else throw new OptimizerException("The passed expression is not an instance of IloNumExpr");
		} catch (IloException e) {
			throw new OptimizerException(e.getMessage());
		}
	}
