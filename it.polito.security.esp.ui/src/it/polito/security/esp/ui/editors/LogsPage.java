...
public class LogsPage extends FormPage
{
	...
	public LogsPage(FormEditor editor, IContainer container) throws Exception
	{
		...
		final IFile logFile = container.getFile(Path.fromOSString("esp.log"));
		...
		logger.addHandler(new Handler()
		{
			...
			{
				if (record.getLoggerName().startsWith("it.polito"))
					try
					{
						logs.append("<p>");
						...
						logs.append("[" + new Date() + "] "
								+ record.getMessage().replaceAll("&", "&amp;").replaceAll("<", "&lt;").replaceAll(">", "&gt;"));
						logs.append("</span>");
						logs.append("</p>");
